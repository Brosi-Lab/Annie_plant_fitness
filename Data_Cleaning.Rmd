...
rm(list = ls())
#Importing Data
soil_moisture_attempt = read.csv("./Raw_data/adjusted_soil_moisture.csv")
```
...
soil.summary$variable<-str_replace(soil.summary$variable,"X","")
ggplot(soil.summary,aes(x=variable,y=value, group = interaction(site,plot.treat))) +
  ...
  labs(y="Soil Moisture (%VWC)",x="Week",title="Soil Moisture in Accelerated Snowmelt and Control Plots") +
...
soil_moisture_attempt$effect.start.date<-as.Date(strptime(soil_moisture_attempt$meltout.date,format = "%m/%d/%Y")+days(14))
```
...
Mertensia <- read.csv("./Raw_data/Mertensia.csv")
```
...
Mertensia$seed.weight<-round(Mertensia$seed.weight,digits=3)
...
Mertensia$start.estimate<-as.Date(floor_date(strptime(Mertensia$bloom.start,format = "%m/%d/%y"), "3 days"))
Mertensia$end.estimate<-as.Date(floor_date(strptime(Mertensia$bloom.end,format = "%m/%d/%y"), "3 days"))
head(Mertensia)
```
...
#we decided not to include soil moisture variables
...
Mertensia.final<-subset(Mertensia,select = -c(date.oe1,date.harvest,total.fruits,ovule.exp1,fruit.collected,height,notes,quad))
head(Mertensia.final)
```

## Getting Relative Position: Control Plot Peak as Population Peak
...
flower.counts<-read.csv("./Raw_data/flower_counts.csv")

#Excluding data from other years-- done previously

#flower.counts<-flower.counts[c(2659:10715),]
#write.csv(flower.counts, "flower_counts.csv")
```
...
Mert.counts<-flower.counts[(flower.counts$plant=="Mertensia fusiformis"),]
Mert.counts<-Mert.counts[,-c(1,9)]
Mert.counts<-na.omit(Mert.counts)
Mert.counts.control<-Mert.counts[(Mert.counts$plot=="Control"),]
```
...
**in revisions for AJB, Mertensia individual midpoints/peaks were changed to 1/3 through blooming** change back to "/2" in following code chunk if not going to use
...
#done with third way through blooming
Mertensia.final$midpoint<- Mertensia.final$start.estimate + floor((Mertensia.final$end.estimate-Mertensia.final$start.estimate)/3)
...
Mert.counts.control.SF<-Mert.counts.control[(Mert.counts.control$site=="Stupid Falls"),]
Mert.counts.control.SF[(which.max(Mert.counts.control.SF$total_flowers)),1]
#Converting dataframe cell to Julian date
Mert.SF.control.max<-julian(as.Date(as.matrix(Mert.counts.control.SF[(which.max(Mert.counts.control.SF$total_flowers)),1])),origin=as.Date("2019-01-01"))
```
...
Mertensia.SF$relative.position<-(Mertensia.SF$julian.midpoint-Mert.SF.control.max)
```
...
Mert.counts.control.WG<-Mert.counts.control[(Mert.counts.control$site=="Wash 3C"),]
Mert.counts.control.WG[(which.max(Mert.counts.control.WG$total_flowers)),1]
Mert.WG.control.max<-julian(as.Date(as.matrix(Mert.counts.control.WG[(which.max(Mert.counts.control.WG$total_flowers)),1])),origin=as.Date("2019-01-01"))
```

```{r calculating Mertensia WG differences}
Mertensia.WG<-Mertensia.final[(Mertensia.final$site=="WG3C"),]
Mertensia.WG$relative.position<-(Mertensia.WG$julian.midpoint-Mert.WG.control.max)
```

### Pfeiler

```{r getting Mertensia PF community peak}
Mert.counts.control.PF<-Mert.counts.control[(Mert.counts.control$site=="Pfeiler"),]
Mert.counts.control.PF[(which.max(Mert.counts.control.PF$total_flowers)),1]
Mert.PF.control.max<-julian(as.Date(as.matrix(Mert.counts.control.PF[(which.max(Mert.counts.control.PF$total_flowers)),1])),origin=as.Date("2019-01-01"))
```

```{r calculating Mertensia PF differences}
Mertensia.PF<-Mertensia.final[(Mertensia.final$site=="Pfeiler"),]
Mertensia.PF$relative.position<-(Mertensia.PF$julian.midpoint-Mert.PF.control.max)
```

### Avery Picnic

```{r getting Mertensia AP community peak}
Mert.counts.control.AP<-Mert.counts.control[(Mert.counts.control$site=="Avery Picnic"),]
Mert.counts.control.AP[(which.max(Mert.counts.control.AP$total_flowers)),1]
Mert.AP.control.max<-julian(as.Date(as.matrix(Mert.counts.control.AP[(which.max(Mert.counts.control.AP$total_flowers)),1])),origin=as.Date("2019-01-01"))
```

```{r calculating Mertensia AP differences}
Mertensia.AP<-Mertensia.final[(Mertensia.final$site=="AveryPicnic"),]
Mertensia.AP$relative.position<-(Mertensia.AP$julian.midpoint-Mert.AP.control.max)
```
...
Mertensia.final2<-rbind(Mertensia.PF,Mertensia.AP,Mertensia.SF,Mertensia.WG)
head(Mertensia.final2)
```

```{r export Mertensia csv}
#rewrote mertensia with 1/3 through bloom time

#write.csv(Mertensia.final2, "Mertensia_final3.csv")
```

## Getting Relative Position: Separate Control and Manip Population Peaks

If we wanted to look use the manipulated plot peak for manipulated Mertensia individuals, we would separate the flower counts into manipulated and control plots.

```{r separating manip and control}
#Mert.counts.manip<-Mertensia.counts[(Mertensia.counts$plot=="Manipulated"),]
```

### Stupid Falls

```{r getting Mertensia SF control and manip peaks}
#Finding date with max number of flowers
#Mert.counts.SF.manip<-Mert.counts.manip[(Mert.counts.manip$site=="Stupid Falls"),]
#Mert.counts.SF.manip[(which.max(Mert.counts.SF.manip$total_flowers)),1]

#Converting dataframe cell to Julian date
#Mert.SF.manip.max<-julian(as.Date(as.matrix(Mert.counts.SF.manip[(which.max(Mert.counts.SF.manip$total_flowers)),1])),origin=as.Date("2019-01-01"))
```

```{r Mert SF relative position}
#Mert.SF.control<-Mertensia.final[(Mertensia.final$site=="StupidFalls") & (Mertensia.final$plot.treat=="control"),]

#Mert.SF.manip<-Mertensia.final[(Mertensia.final$site=="StupidFalls") & (Mertensia.final$plot.treat=="manip"),]

#Mert.SF.control$relative.position<-(Mertensia.SF$julian.midpoint-Mert.SF.control.max)
Mert.SF.manip$relative.position<-(Mertensia.SF$julian.midpoint-Mert.SF.manip.max)

#Mert.SF.modified.peaks<-rbind(Mert.SF.control,Mert.SF.manip)
```
...
Potentilla <- read.csv("./Raw_data/Potentilla.csv")
```
...
Potentilla.final$end.estimate<-as.Date(ifelse(is.na(Potentilla.final$end.estimate), Potentilla.final$end.estimate.modified, Potentilla.final$end.estimate), origin= "2019-01-01") #compiling the calculated end estimates and actual end estimates
```
...
#Finding midpoint between start and end estimates
Potentilla.final$midpoint<- Potentilla.final$start.estimate + floor((as.POSIXct(Potentilla.final$end.estimate, format= "%Y-%m-%d")-Potentilla.final$start.estimate)/2)
...
Delphinium <- read.csv("./Raw_data/Delphinium.csv")
```
