...
aggregate(x=mert.flower.counts$total_flowers,by=list(mert.flower.counts$date,mert.flower.counts$site),FUN="mean") #calculate mean flower counts by week and site
mert$conspecifics<-
```
...
ggplot(mert, aes(x=mert$deviation, y=mert$total.seeds, group= mert$early.late)) +
  ...
  geom_smooth(method="glm",aes(color=early.late),yintercept=mean(mert$total.seeds[mert$deviation==0]))
```
