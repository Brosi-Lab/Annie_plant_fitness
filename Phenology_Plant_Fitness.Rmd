...
mert<-mert[!is.na(mert$seeds.per.flower),] # removing NA from seeds per flower column
...
delph$seeds.per.flower<-((delph$dev.seed+delph$dev.seed1)/delph$X.flowers)
delph<-delph[!is.na(delph$early.late),]
delph<-delph[!is.na(delph$seeds.per.flower),] #removing NA values in seeds per flower column
...
pot$seeds.per.flower<-((pot$dev.seed.sharp+pot$dev.seed.rest)/(pot$X.flowers+pot$fl.count.sharp))
...
pot<-pot[!is.na(pot$seeds.per.flower),] #removing NA values in seeds per flower column
```
...
# Mixed Effects Models: Effect of Soil Moisture Variables on Total Seeds and Seeds per Flower
...
First, we combined the variables for the three species in order to run the model selection with all of the collected seeds.
...
We checked the distribution of the total number of developed seeds and found that they fit a negative binomial distribution.

```{r checking distribution for total seed data}
...
Next we checked the distribution for seeds per flower and found that they fit an exponential distribution.
```{r checking distribution for seeds per flower data}
...
Because the total seeds fit a negative binomial distribution, we used the glmmTMB package. Because the seeds per flower variable fit a exponential distribution, we used the lme4 package.
...
```{r soil model with lme4, warning=FALSE}
all.seeds$non_zero <- ifelse(all.seeds$seeds.per.flower > 0, 1, 0) # creating zero/non-zero column
soil.mod2.hurdle<-glmer(non_zero ~ mean + rate + effect.min + end + range + (1|site/plot.treat) + (1|species), data=all.seeds,family = binomial(link = logit)) # model with new column
soil.mod2.gamma<-glmer(seeds.per.flower ~ mean + rate + effect.min + end + range + (1|site/plot.treat) + (1|species), family=Gamma(link = log),data = subset(all.seeds,non_zero==1)) # model without zeros

#summary(soil.mod2.hurdle)
summary(soil.mod2.gamma)
```

Finally, we ran two models selections: one for the glmmTMB model with the count data and one for the glmer model with the seeds/flower data.
...
```{r model 2 selection with dredge, message=FALSE}
dredge_result2<-dredge(soil.mod2) #using dredge function for model selection
subset(dredge_result2, delta<4)
```
After running the model selections, we found that rate of soil moisture was the best model for the seeds/flower response variable and the end soil moisture was the best model for the seed count data. 
...
```
The Potentilla pollen limitation data were not overdispersed, but they are slightly zero-inflated. We addressed the zero-inflation issue by again including the zero-inflation formula into our models. 
...
# Mixed Effects Models with Seeds per Flower Data

```{r limiting Mertensia to open again}
mert.open<-mert[(mert$treat=="open"),] #removed hand pollination treatment
```

```{r checking Mertensia distribution again}
mert.norm<-fitdist(mert.open$seeds.per.flower, "gamma") #fitting Mertensia data to normal distribution using fitdistplus
plot(mert.norm) #plotting to see the fit
```

The Mertensia seed data fits a gaussian distribution. The lmerTEST package is a good package for modeling data with a gaussian distribution. The lme4 package is also suitable for data that could be overdispersed or zero-inflated.

```{r Mertensia glmer with relative position, warning=FALSE}
mert.glmer<-glmer(seeds.per.flower ~ deviation * early.late + plot.treat * number.conspecifics + rate + (1|site), family="Gamma", data = mert.open) #putting seeds per flower data into lmer for DHARMa package
summary(mert.glmer) #summary of glmer model
```

```{r checking mertensia seeds per flower overdispersion}
mert.seeds.flower.simulation<-simulateResiduals(fittedModel = mert.lmer) #creating simulation for Mertensia mixed effects model with DHARMa package
plot(mert.seeds.flower.simulation) #plotting simulation
testDispersion(mert.seeds.flower.simulation) #checking for overdispersion
testZeroInflation(mert.seeds.flower.simulation) #checking for zero inflation
```
It is zero-inflated. We rewrote the model to account for zero-inflation.

```{r Mertensia glmer with zero inflation, warning=FALSE}
mert.lmer<-lmer(seeds.per.flower ~ deviation * early.late + plot.treat * number.conspecifics + rate + (1|site), data = mert.open) #putting seeds per flower data into lmer for DHARMa package
summary(mert.lmer) #summary of glmer model
```
