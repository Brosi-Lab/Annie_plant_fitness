...
```{r combining all of the seed data, warning=FALSE, message=FALSE}
...
```{r soil model with glmmtmb, warning=FALSE, message=FALSE}
...
```{r hurdle model with lme4, warning=FALSE, message=FALSE}
...
```{r model selection with dredge, message=FALSE, warning=FALSE}
...
```{r limiting Mertensia to open, warning=FALSE, message=FALSE}
...
```{r limiting Delphinium to open, warning=FALSE,message=FALSE}
...
```{r limiting Potentilla to open, warning=FALSE,message=FALSE}
...
We did detected a significant effect of pollen supplementation on the proportions of developed seeds for Mertensia individuals, as well as a marginally significant interaction between flower number and pollination treatment. We again checked to see if the data were overdispersed or zero-inflated, and they were not. **In conjunction with the seed count results above, it is clear that Mertensia individuals are pollen limited.**
...
```{r delphinium proportion model, warning=FALSE,message=FALSE}
#Delph.prop.glmmtmb<-glmmTMB(cbind(dev.seed.total,undev.seed.total) ~ X.flowers:(deviation * early.late) + X.flowers:number.conspecifics/plot.treat + (1|site), family = binomial, data = delph.open) #modeling Delphinium proportion of developed seeds with glmmTMB package
...
Our model shows that the interation between number of conspecifics and plot treatment, as well as the interaction between number of flowers and deviation, had an effect on seed set. However, the data are zero-inflated and overdispersed. We adjusted our model to account for this.

```{r new delph model, warning=FALSE, message=FALSE}
Delph.prop.glmmtmb<-glmmTMB(cbind(dev.seed.total,undev.seed.total) ~ X.flowers:(deviation * early.late) + X.flowers:number.conspecifics/plot.treat + (1|site), ziformula=~1, family = betabinomial, data = delph.open) #modeling Delphinium proportion of developed seeds with betabinomial family and ziformula
...
testZeroInflation(delph.prop.simulation) #checking for zero inflation
```
After correcting for overdispersion and zero-inflation, we did not detect an effect of blooming time or conspecific density.
...
```{r delph prop pollen lim,warning=FALSE, message=FALSE}
...
Delphinium proportions of developed seeds were significantly affected by pollen supplementation, but the pollen limitation model for Delphinium proportion data is zero-inflated. As with the model above, we changed the family to beta binomial and included the zero-inflation formula.

```{r new prop pollen lim model, warning=FALSE, message=FALSE}
...
```{r main hurdle model with lme4, warning=FALSE, eval=FALSE}
...
```{r delph hurdle model with lme4, warning=FALSE, message=FALSE}
...
```{r pot hurdle model with lme4, warning=FALSE,eval=FALSE}
