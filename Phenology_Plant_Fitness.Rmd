...
soil.mod2.gamma<-glmer(seeds.per.flower ~ mean + rate + effect.min + end + range + (1|site/plot.treat) + (1|species), family=Gamma,data = subset(all.seeds,non_zero==1)) # model without zeros
...
```
We detected an effect of pollen limitation on the total developed seeds of Delphinium individuals.
...
```
The Delphinum pollen limitation model is not overdispersed. The data are zero-inflated. Because the data were zero-inflated, we adjusted our model to include the zero-inflation formula and re-tested the overdispersion and zero-inflation.
...
testZeroInflation(delph.counts.treat.simulation) #checking for zero inflation
```
The new model is not overdispersed or zero-inflated. We still see an effect of pollen limitation on the Delphinium count data.
...
We detected a marginally significant effect of the pollen limitation treatment on the total number of developed seeds for Potentilla individuals.
...
```
The Potentilla pollen limitation data were overdispersed and zero-inflated. We addressed the zero-inflation issue by again including the zero-inflation formula into our models. 
...
testZeroInflation(pot.counts.treat.simulation) #checking for zero inflation
```
The data are no longer zero-inflated or overdispersed, and we still detected a marginally significant effect of pollen limitation in the Potentilla individuals.
...
summary(Delph.prop.treat.glmmtmb) #model summary
```
After correcting the model, we detected a significant effect of pollen limitation on the proportion of developed seeds for Delphinium individuals. Again we checked for overdispersion and zero-inflation.
