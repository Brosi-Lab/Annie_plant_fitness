...
mert<-mert[(mert$treat=="open"),] #removed hand pollination treatment
```
...
mert.glmmadmb<-glmmadmb(dev.seed ~ deviation * early.late + plot.treat * number.conspecifics + (1|site), family = "nbinom2", data = mert) #Mertensia model for negative binomial data with glmmadmb package
...
delph<-delph[(delph$treat=="open"),] #removed hand pollination treatment
```
...
delph.glmmadmb<-glmmadmb(total.seeds ~ deviation * early.late + plot.treat * number.conspecifics + (1|site), family = "nbinom2", data = delph) #Delphinium model for negative binomial data with glmmadmb package
...
pot<-pot[(pot$treat=="open"),] #removed hand pollination treatment
```
...
```{r Potentilla glmm}
pot.glmmadmb<-glmmadmb(total.seeds ~ deviation * early.late + plot.treat * number.conspecifics + plot.treat + (1|site), family = "nbinom2", data = pot) #Potentilla model for negative binomial data with glmmadmb package
#removed treat fixed effect (only open)
...
Mert.glmer<-glmer(cbind(dev.seed,undev.seeds) ~ deviation * early.late + plot.treat * number.conspecifics + (1|site), family = binomial, data = mert) #Mertensia model with proportion of developed seeds using lme4 package
#removed treat fixed effect (only open in data set)
summary(Mert.glmer)
```
...
Delph.glmer<-glmer(cbind(dev.seed.total,undev.seed.total) ~ deviation * early.late + plot.treat * number.conspecifics + (1|site), family = binomial, data = delph) #modeling Delphinium proportion of developed seeds with lme4 package
#removed treat fixed effect (only open)
summary(Delph.glmer)
```
