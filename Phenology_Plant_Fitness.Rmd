...
delph<-delph[!is.na(delph$total.seeds),]
...
pot$fl.count.sharp[is.na(pot$fl.count.sharp)] <- 0 #converting NA to 0
...
```{r hurdle model with lme4, warning=FALSE}
...
#mert.glmmtmb<-glmmTMB(dev.seed ~ flowers:deviation * early.late + number.conspecifics/plot.treat + rate + (1|site), family = "nbinom2", data = mert.open) #putting count data into glmmTMB for DHARMa package
...
We did not detect an effect of relative blooming time, conspecific density, or plot treatment on the total number of seeds in Mertensia individuals. We then checked for overdispersion and zero-inflation in the data using the DHARMa package. The overdispersion and zero-inflation tests work with the glmmTMB package. The Mertensia count data is not overdispersed, but it is slightly zero-inflated. To address this, we adjusted our glmmtmb model to include zero-inflation.

```{r correcting mert counts glmmtmb}
mert.glmmtmb<-glmmTMB(dev.seed ~ flowers:(deviation*early.late) + flowers:number.conspecifics/plot.treat + rate:flowers + (1|site), ziformula=~1, family = "nbinom2", data = mert.open) #putting count data into glmmTMB for DHARMa package
summary(mert.glmmtmb) #summary of glmmTMB model
...
testZeroInflation(mert.counts.simulation) #checking for zero inflation
```
When number of flowers is taken into account, seed set for Mertensia individuals were affected by deviation from the population peak. Zero-inflation is accounted for in the new model, and the model for Mertensia count data is no longer zero-inflated. 

To figure out the direction of the effect, we looked at the plots (shown below). **Deviation from the population peak has a positive effect on seed set. As deviation increases, seed set increases.**
...
We detected an effect of pollen limitation treatment on Mertensia total developed seeds, but we had to test the model assumptions of our pollen limitation model as well. After converting our model to an glmmTMB model, we tested for overdispersion and zero-inflation with the DHARMa package. The data in the pollen limitation model for Mertensia are not overdispersed, but they are zero-inflated. We then adjusted our model to include zero-inflation and ran the overdispersion and zero-inflation tests again.
...
delph.glmmtmb<-glmmTMB(total.seeds ~ X.flowers:deviation*early.late + number.conspecifics/plot.treat + rate + (1|site), family = "nbinom2", data = delph.open) #Delphinium model for negative binomial data with glmmTMB package
...
Soil moisture rate had an effect on Delphinium individual seed set. Additionally, when flower number is taken into account, deviation from the population peak has a marginally significant effect on Delphinium number of developed seeds. We tested the model assumptions as we did with Mertensia seed count data. We checked for overdispersion and zero-inflation. The model for the Delphinium count data is not overdispersed or zero-inflated.
...
#pot.glmmtmb<-glmmTMB(total.seeds ~ X.flowers:deviation * early.late + number.conspecifics/plot.treat + rate + (1|site), family = "nbinom2", data = pot.open) #Potentilla model for negative binomial data with glmmTMB package
...
```
We detected a marginally significant effect of soil moisture rate on the total developed seeds of Potentilla individuals. The Potentilla data are not overdispersed, but they are zero-inflated. To fix this, we had to include zero inflation in our model and again check for zero-inflation.

```{r Potentilla new glmmtmb,warning=FALSE}
pot.glmmtmb<-glmmTMB(total.seeds ~ X.flowers:deviation * early.late + number.conspecifics/plot.treat + rate + (1|site), ziformula=~1, family = "nbinom2", data = pot.open) #new Potentilla model for negative binomial data with glmmTMB package
summary(pot.glmmtmb) #summary of model output
...
testZeroInflation(pot.counts.simulation) #checking for zero inflation
```
When we incorporated zero-inflation into the model, we found a marginally significant trend in the soil moisture variable. As the rate of change in soil moisture increases, the number of developed seeds in Potentilla individuals decreases. Additionally, we detected an effect between deviation, early/late, and number of flowers. The direction of the effect, however, must be determined from the plots (below).
...
We detected a marginally significant effect of the pollen limitation treatment on the total number of developed seeds for Potentilla individuals. The Potentilla pollen limitation data were overdispersed and zero-inflated. We addressed the zero-inflation issue by again including the zero-inflation formula into our models. 
...
Mert.prop.glmmtmb<-glmmTMB(cbind(dev.seed,undev.seeds) ~ flowers:deviation * early.late + number.conspecifics/plot.treat + rate + (1|site), family = binomial, data = mert.open) #Mertensia model with proportion of developed seeds using the glmmTMB package
...
A significant effect was detected in the interaction between conspecifics and plot treatment, the rate of change in soil moisture, and the interaction between number of flowers, early/late, and deviation. As the number of conspecifics increased, the proportion of developed seeds in Mertensia individuals decreased, with a greater decline in the accelerated snowmelt plot. For the soil moisture variable, as the rate of change in soil moisture increased, the proportion of developed seeds decreased. Finally, when the number of flowers was taken into account, we detected an effect of deviation and early/late. The direction, however, had to be determined using the plots below. **When flower number was taken into account, deviation had a greater positive effect on late-blooming Potentilla individuals.**

Once we had our model, we were able to check for overdispersion and zero inflaction. We checked for overdispersion and zero inflation with the DHARMa package.The Mertensia proportion data were not overdispersed or zero-inflated.
...
We did detected an marginally significant effect of pollen supplementation on the proportions of developed seeds for Mertensia individuals. We again checked to see if the data were overdispersed or zero-inflated, and they were not. **In conjunction with the developed seed counts above, it is clear that Mertensia individuals are pollen limited.**
...
```{r delphinium proportion model, warning=FALSE,message=FALSE}
#Delph.prop.glmmtmb<-glmmTMB(cbind(dev.seed.total,undev.seed.total) ~ X.flowers:deviation * early.late + number.conspecifics/plot.treat + rate + (1|site), family = binomial, data = delph.open) #modeling Delphinium proportion of developed seeds with glmmTMB package
...
Our model shows that the number of conspecifics and interaction between number of flowers and deviation had an effect on seed set. We again tested for overdispersion and zero inflation. We used the DHARMa package for the Delphinium proportion data, and the data are zero-inflated. We adjusted our model to account for this.

```{r new delph model, warning=FALSE, message=FALSE}
Delph.prop.glmmtmb<-glmmTMB(cbind(dev.seed.total,undev.seed.total) ~ X.flowers:deviation * early.late + number.conspecifics/plot.treat + rate + (1|site), ziformula=~1, family = betabinomial, data = delph.open) #modeling Delphinium proportion of developed seeds with betabinomial family and ziformula
...
testZeroInflation(delph.prop.simulation) #checking for zero inflation
```
After correcting for overdispersion and zero-inflation, we detected a marginally significant of deviation on the proportion of developed seeds for Delphinium individuals. We confirmed with the models below, and **the Delphinium individuals that deviated from the population peak had lower seed set when number of flowers was taken into account.** We again checked for overdispersion and zero-inflation in the new model, and the data are no longer overdispersed or zero-inflated.
...
#Delph.prop.treat.glmmtmb<-glmmTMB(cbind(dev.seed.total,undev.seed.total) ~ treat/X.flowers + (1|site/plot.treat), family = binomial, data = delph) #modeling pollen lim and proportion of developed seeds with glmmTMB package
...
Delphinium proportions of developed seeds were significantly affected by pollen supplementation. The pollen limitation model for Delphinium proportion data is zero-inflated. As with the model above, we changed the family to beta binomial and included the zero-inflation formula.

```{r new prop pollen lim model}
Delph.prop.treat.glmmtmb<-glmmTMB(cbind(dev.seed.total,undev.seed.total) ~ treat/X.flowers + (1|site/plot.treat), ziformula=~1, family = betabinomial, data = delph) #modeling pollen lim and proportion of developed seeds with glmmTMB
summary(Delph.prop.treat.glmmtmb) #model summary
...
After correcting the model, we detected a significant effect of pollen limitation on the proportion of developed seeds for Delphinium individuals. Again we checked for overdispersion and zero-inflation, and the data are no longer overdispersed or zero-inflated. **As shown with total developed seeds, Delphinium individuals are pollen limited in this system.**
...
ggplot(delph.open, aes(x=delph.open$deviation, y=(delph.open$total.seeds/(delph.open$total.seeds+delph.open$undev.seed.total))/delph.open$X.flowers, group= delph.open$early.late)) +
...
mert.open<-mert.open[!(mert.open$seed.weight=="0"),] #removing individuals without developed seed weight
mert.open$mean.weight<-mert.open$seed.weight/mert.open$dev.seed #finding mean weight
```
...
mert.gaus<-fitdist(mert.open$mean.weight, "norm") #fitting Mertensia data to normal distribution using fitdistplus
...
mert.lmer<-lmer(mean.weight ~ deviation * early.late + plot.treat * number.conspecifics + rate + (1|site), data = mert.open) #Mertensia model with seed weights using the lmerTest package
summary(mert.lmer)
```
...
mert$seed.weight<-mert$seed.weight*1000 #converting seed weights to mg
mert<-mert[!(mert$seed.weight=="0"),] #removing individuals without developed seed weight
mert$mean.weight<-mert$seed.weight/mert$dev.seed #finding mean weight
```

```{r mert pollen lim seed weights, message=FALSE, warning=FALSE}
mert.treat.lmer<-lmer(mean.weight ~ treat + (1|site/plot.treat), data = mert) #Mertensia model for seed weight data and pollen lim treatment with lme4 package
...
delph.open<-delph.open[!(delph.open$seed.weight=="0"),] #removing individuals without developed seed weight
delph.open$mean.weight<-delph.open$seed.weight/delph.open$total.seeds #finding mean weight
delph.open<-delph.open[-14,] #removing an outlier
```
...
delph.gaus<-fitdist(delph.open$mean.weight, "norm") #fitting Mertensia data to normal distribution using fitdistplus
...
delph.lmer<-lmer(mean.weight ~ deviation * early.late + plot.treat * number.conspecifics + rate + (1|site), data = delph.open) #Delphinium model with seed weights using the lmerTest package
summary(delph.lmer)
```
We did not detect an effect of relative blooming time, conspecific density, or soil moisture on the seed weights of Delphinium individuals.
...
delph<-delph[!(delph$seed.weight=="0"),] #removing individuals without developed seed weight
delph$mean.weight<-delph$seed.weight/delph$total.seeds #finding mean weight
```

```{r delph pollen lim seed weights, message=FALSE, warning=FALSE}
delph.treat.lmer<-lmer(mean.weight ~ treat + (1|site/plot.treat), data = delph) #Delphinium model for seed weight data and pollen lim treatment with lme4 package
...
pot.open<-pot.open[!(pot.open$total.seed.weight=="0"),] #removing individuals without developed seed weight
pot.open$mean.weight<-pot.open$total.seed.weight/pot.open$total.seeds #finding mean weight
```

We then checked the distribution. Because weight is a continuous variable, the data fit a gaussian distribution.

```{r checking pot weight seed distribution, message=FALSE, warning=FALSE}
pot.gaus<-fitdist(pot.open$mean.weight, "norm") #fitting Potentilla data to normal distribution using fitdistplus
...
pot.lmer<-lmer(mean.weight ~ deviation * early.late + plot.treat * number.conspecifics + rate + (1|site), data = pot.open) #Potentilla model with seed weights using the lmerTest package
summary(pot.lmer)
```
...
pot$total.seed.weight<-pot$total.seed.weight*1000 #converting seed weights to mg
pot<-pot[!(pot$total.seed.weight=="0"),] #removing individuals without developed seed weight
pot$mean.weight<-pot$total.seed.weight/pot$total.seeds #finding mean weight
```

```{r pot pollen lim seed weights, message=FALSE, warning=FALSE}
pot.treat.lmer<-lmer(mean.weight ~ treat + (1|site/plot.treat), data = pot) #Potentilla model for seed weight data and pollen lim treatment with lme4 package
summary(pot.treat.lmer) #summary of model output
```
Pollen-supplemented Potentilla individuals had lower seed weight than open individuals. We checked for overdispersion and zero-inflation again.
