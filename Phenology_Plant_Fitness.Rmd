...
mert<-mert[!is.na(mert$seeds.per.flower),] #removing NA values from Mertenia seeds per flower column
...
delph$seeds.per.flower<-(delph$total.seeds/delph$X.flowers)
delph<-delph[!is.na(delph$seeds.per.flower),] #removing NA values from Mertenia seeds per flower column
...
pot$seeds.per.flower<-(pot$total.seeds/pot$number.flowers)
pot<-pot[!is.na(pot$seeds.per.flower),] #removing NA values from Mertenia seeds per flower column
...
mert.nbinom<-fitdist(mert$dev.seed, "nbinom") #fitting Mertensia data to negative binomial distribution using fitdistplus
...
mert.glmmadmb<-glmmadmb(dev.seed ~ deviation * early.late + plot.treat * number.conspecifics + treat + (1|site), family = "nbinom2", data = mert) #Mertensia model for negative binomial data with glmmadmb package
...
```{r Mertensia plot}
ggplot(mert, aes(x=mert$relative.position, y=mert$dev.seed, group=mert$plot.treat)) +
...
ggplot(mert, aes(x=mert$deviation, y=mert$dev.seed, group= mert$early.late)) +
...
ggplot(mert, aes(x=mert$number.conspecifics, y=mert$dev.seed, group= mert$plot.treat)) +
...
```{r Mertensia seeds per flower model}
Mert.seeds.flower.glmer<-glmer(cbind(dev.seed,flowers) ~ deviation * early.late + plot.treat * number.conspecifics + treat + (1|site/flowers), family = binomial, data = mert) #Mertensia model with seeds per flower using lme4 package
summary(Mert.seeds.flower.glmer)
```

```{r Mertensia seeds per flower relative position plot}
ggplot(mert, aes(x=mert$relative.position, y=mert$seeds.per.flower, group=mert$plot.treat)) +
  labs(y="Seeds per Flower",x="Days Relative to Population Peak Bloom", title="Effect of Mertensia Blooming Time on Seeds per Flower") +
  geom_point(aes(color=plot.treat),size=3, alpha = 0.8) +
  scale_color_brewer(palette="Dark2") + labs(color="Plot Treatment") +
  scale_color_manual(labels= c("control","accelerated"), values = c("control"="darkred","manip"="darkorange")) +
  geom_smooth()
...
```{r Mertensia deviation plot seeds per flower}
ggplot(mert, aes(x=mert$deviation, y=mert$seeds.per.flower, group= mert$early.late)) +
  labs(y="Seeds per Flower",x="Days Relative to Population Peak Bloom", title="Effect of Mertensia Deviation from Peak on Seeds per Flower") + 
  theme_classic() +
  geom_point(size=3, alpha = 0.8, aes(color=early.late)) +
  geom_smooth(method="glm",aes(color=early.late)) +
  scale_color_brewer(palette="Dark2") + 
...
```{r Mertensia seeds per flower plot with conspecific density}
ggplot(mert, aes(x=mert$number.conspecifics, y=mert$seeds.per.flower, group= mert$plot.treat)) +
  labs(y="Seeds per Flower",x="Conspecific Density", title="Effect of Mertensia Conspecific Density on Seeds per Flower") +
  theme_classic() +
  geom_point(size=3, alpha = 0.8, aes(color=plot.treat)) +
  geom_smooth(method="glm",aes(color=plot.treat)) +
  scale_color_brewer(palette="Dark2") + 
...
```{r Delphinium seeds per flower model}
Delph.seeds.flower.glmer<-glmer(cbind(total.seeds,X.flowers) ~ deviation * early.late + plot.treat * number.conspecifics + treat + (1|site/X.flowers), family = binomial, data = delph) #Delphinium model with seeds per flower using lme4 package
summary(Delph.seeds.flower.glmer)
```

```{r Delphinium seeds per flower relative position plot}
ggplot(delph, aes(x=delph$relative.position, y=delph$seeds.per.flower, group=delph$plot.treat)) +
  labs(y="Seeds per Flower",x="Days Relative to Population Peak Bloom", title="Effect of Delphinium Blooming Time on Seeds per Flower") +
  geom_point(aes(color=plot.treat),size=3, alpha = 0.8) +
  scale_color_brewer(palette="Dark2") + labs(color="Plot Treatment") +
  scale_color_manual(labels= c("control","accelerated"), values = c("control"="darkred","manip"="darkorange")) +
  geom_smooth()
...
```{r Delphinium deviation plot seeds per flower}
ggplot(delph, aes(x=delph$deviation, y=delph$seeds.per.flower, group= delph$early.late)) +
  labs(y="Seeds per Flower",x="Days Relative to Population Peak Bloom", title="Effect of Delphinium Deviation from Peak on Seeds per Flower") + theme_classic() +
  geom_point(size=3, alpha = 0.8, aes(color=early.late)) +
  geom_smooth(method="glm",aes(color=early.late)) +
  scale_color_brewer(palette="Dark2") + 
  labs(color="Before or After Peak") +
...
```{r Delphinium seeds per flower plot with conspecific density}
ggplot(delph, aes(x=delph$number.conspecifics, y=delph$seeds.per.flower, group= delph$plot.treat)) +
  labs(y="Seeds per Flower",x="Conspecific Density", title="Effect of Delphinium Conspecific Density on Seeds per Flower") +
  theme_classic() +
  geom_point(size=3, alpha = 0.8, aes(color=plot.treat)) +
  geom_smooth(method="glm",aes(color=plot.treat)) +
  scale_color_brewer(palette="Dark2") + 
...
```{r Potentilla seeds per flower model}
Pot.seeds.flower.glmer<-glmer(cbind(total.seeds,number.flowers) ~ deviation * early.late + plot.treat * number.conspecifics + treat + (1|site/number.flowers), family = binomial, data = pot) #Potentilla model with seeds per flower using lme4 package
summary(Pot.seeds.flower.glmer)
```

```{r Potentilla seeds per flower relative position plot}
ggplot(pot, aes(x=pot$relative.position, y=pot$seeds.per.flower, group=pot$plot.treat)) +
  labs(y="Seeds per Flower",x="Days Relative to Population Peak Bloom", title="Effect of Potentilla Blooming Time on Seeds per Flower") +
  geom_point(aes(color=plot.treat),size=3, alpha = 0.8) +
  scale_color_brewer(palette="Dark2") + labs(color="Plot Treatment") +
  scale_color_manual(labels= c("control","accelerated"), values = c("control"="darkred","manip"="darkorange")) +
  geom_smooth()
...
```{r Potentilla deviation plot seeds per flower}
ggplot(pot, aes(x=pot$deviation, y=pot$seeds.per.flower, group= pot$early.late)) +
  labs(y="Seeds per Flower",x="Days Relative to Population Peak Bloom", title="Effect of Potentilla Deviation from Peak on Seeds per Flower") + theme_classic() +
  geom_point(size=3, alpha = 0.8, aes(color=early.late)) +
  geom_smooth(method="glm",aes(color=early.late)) +
  scale_color_brewer(palette="Dark2") + 
  labs(color="Before or After Peak") +
...
```{r Potentilla seeds per flower plot with conspecific density}
ggplot(pot, aes(x=pot$number.conspecifics, y=pot$seeds.per.flower, group= pot$plot.treat)) +
  labs(y="Seeds per Flower",x="Conspecific Density", title="Effect of Potentilla Conspecific Density on Seeds per Flower") +
  theme_classic() +
  geom_point(size=3, alpha = 0.8, aes(color=plot.treat)) +
  geom_smooth(method="glm",aes(color=plot.treat)) +
  scale_color_brewer(palette="Dark2") + 
