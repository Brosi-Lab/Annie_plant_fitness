...
all.seeds$seeds.per.flower[all.seeds$seeds.per.flower==0]<-0.00001
soil.mod2<-glmer(seeds.per.flower ~ mean + rate + effect.min + end + range + (1|site/plot.treat) + (1|species), family="Gamma",data = all.seeds,na.action = "na.fail") #model for soil moisture variables with all seeds
summary(soil.mod2)
```
...
mert.norm<-fitdist(mert.open$seeds.per.flower, "norm") #fitting Mertensia data to normal distribution using fitdistplus
