...
```
After running the model selection, we found that rate of soil moisture was the best model, but the null model was a very close second. The delta values between the rate of change predictor and the null model are 0.03 apart, which is very little. Because the rate of change variable is not that different than the null model, we will not include the soil moisture predictor into our larger models later on. 
