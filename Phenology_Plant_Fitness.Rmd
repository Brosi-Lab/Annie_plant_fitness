...
We detected an effect of pollen limitation treatment on Mertensia total developed seeds.
...
testZeroInflation(mert.counts.treat.simulation) #checking for zero inflation
```
The data are no longer zero-inflated, and we still detected an effect of pollen limitation on the Mertensia seed counts.
...
```
The Delphinum pollen limitation model is overdispersed and zero-inflated. We adjusted our model to include the zero-inflation formula, and re-tested for overdispersion and zero-inflation.
...
```
We did detected an marginally significant effect of pollen supplementation on the proportions of developed seeds for Mertensia individuals.
...
```{r Mertensia pollen lim with seeds per flower}
mert$non_zero <- ifelse(mert$seeds.per.flower > 0, 1, 0) # creating zero/non-zero column

mert.hurdle<-glmer(non_zero ~ treat + (1|site/plot.treat), data=mert,family = binomial) # model with new column

mert.nonzero = subset(mert,non_zero==1)
mert.mod<-glmer(seeds.per.flower ~ treat + (1|site/plot.treat), family=Gamma, data = mert.nonzero,na.action = "na.fail") # model without zeros

summary(mert.hurdle) # viewing model results
summary(mert.mod)
```

```{r Delphinium pollen lim with seeds per flower}
delph$non_zero <- ifelse(delph$seeds.per.flower > 0, 1, 0) # creating zero/non-zero column

delph.hurdle<-glmer(non_zero ~ treat + (1|site/plot.treat), data=delph,family = binomial) # model with new column

delph.nonzero = subset(delph,non_zero==1)
delph.mod<-glmer(seeds.per.flower ~ treat + (1|site/plot.treat), family=Gamma, data = delph.nonzero,na.action = "na.fail") # model without zeros

summary(delph.hurdle) # viewing model results
summary(delph.mod)
```

```{r Potentilla pollen lim with seeds per flower}
pot$non_zero <- ifelse(pot$seeds.per.flower > 0, 1, 0) # creating zero/non-zero column

pot.hurdle<-glmer(non_zero ~ treat + (1|site/plot.treat), data=pot,family = binomial) # model with new column

pot.nonzero = subset(pot,non_zero==1)
pot.mod<-glmer(seeds.per.flower ~ treat + (1|site/plot.treat), family=Gamma, data = pot.nonzero,na.action = "na.fail") # model without zeros

summary(pot.hurdle) # viewing model results
summary(pot.mod)
```

Seeds per flower not a good predictor of pollen limitation. Interaction with treatment and number of flowers resulted in all species be at least somewhat pollen limited. Is seeds per flower or the interaction more appropriate?
