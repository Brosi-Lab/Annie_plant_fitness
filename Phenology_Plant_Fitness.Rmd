...
```{r checking distribution, warning=FALSE, message=FALSE}
...
```{r checking potentilla distribution, warning=FALSE,message=FALSE}
...
The seed set of Delphinium individuals wasn't significantly affected by relative blooming time and conspecific density. Our model was overdispersed to binomial errors, but not to betabinomial errors. 
**Relative blooming time and conspecific density do not have an effect on the proportion of developed seeds in Delphinium individuals.**
...
## Pollen limitation figures

Some quick bar graphs to determine how the pollen supplementation treatment affected seed set
...
A few last graphs to see the relationship between plot treatment and flower number (on the individuals, not conspecific density flower counts).

```{r plots showing relationship between plot treatment and number of flowers}
