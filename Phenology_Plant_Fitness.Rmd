...
```{r Mertensia glmmadmb with relative position, warning=FALSE}
...
We then checked for overdispersion and zero-inflation in the data using the DHARMa package. The overdispersion and zero-inflation tests work with the glmmTMB package, not the glmmADMB package. To test for these model assumptions, we had to adjust our model and use the glmmTMB mixed effects model.
...
The Mertensia count data is not overdispersed, but it is slightly zero-inflated.
...
```{r Mert glmmadmb pollen treat, warning=FALSE}
...
mert.treat.glmmtmb<-glmmTMB(dev.seed ~ treat + (1|site/plot.treat), family = "nbinom2", data = mert) #putting count data into glmmTMB for DHARMa package
summary(mert.treat.glmmtmb) #summary of glmmTMB model
...
The data in the pollen limitation model for Mertensia is not overdispersed, but it is zero-inflated.
...
The Delphinium data fits a negative binomial distribution. We used the glmmADMB package again.

```{r Delphinium glmmADMB,warning=FALSE}
...
delph.glmmtmb<-glmmTMB(total.seeds ~ deviation * early.late + plot.treat * number.conspecifics + (1|site), family = "nbinom2", data = delph.open) #putting count data into glmmTMB for DHARMa package
summary(delph.glmmtmb) #summary of glmmTMB model
...
```{r Delph glmmadmb pollen treat,warning=FALSE}
...
delph.treat.glmmtmb<-glmmTMB(total.seeds ~ treat + (1|site/plot.treat), family = "nbinom2", data = delph) #putting count data into glmmTMB for DHARMa package
summary(delph.treat.glmmtmb) #summary of glmmTMB model
...
```
The Delphinum pollen limitation model is not overdispersed. The data are slightly zero-inflated.
...
The Potentilla seed data fits a negative binomial distribution. We used the glmmADMB package again.

```{r Potentilla glmm,warning=FALSE}
...
pot.glmmtmb<-glmmTMB(total.seeds ~ deviation * early.late + plot.treat * number.conspecifics + plot.treat + (1|site), family = "nbinom2", data = pot.open) #putting count data into glmmTMB for DHARMa package
summary(pot.glmmtmb) #summary of glmmTMB model
...
```
The Potentilla data are not overdispersed, but they are zero-inflated.
...
```{r Pot glmmadmb pollen treat,warning=FALSE}
pot.treat.glmmadmb<-glmmadmb(total.seeds ~ treat + (1|site/plot.treat), family = "nbinom2", data = pot) #Potentilla model for negative binomial data with glmmadmb package
...
summary(pot.treat.glmmtmb) #summary of glmmTMB model
...
```
The Potentilla pollen limitation data were not overdispersed, but they are zero-inflated.
...
mert.prop.simulation<-simulateResiduals(fittedModel = Mert.glmer) #creating simulation for Mertensia mixed effects model with DHARMa package
...
Finally, we created the pollen limitation model for the Mertensia developed seeds. The model includes site and plot treatment as random effects like the models above. Now the proportion of developed seeds is the response variable.
...
mert.prop.treat.simulation<-simulateResiduals(fittedModel = Mert.treat.glmer) #creating simulation for Mertensia mixed effects model with DHARMa package
...
Delph.glmer<-glmer(cbind(dev.seed.total,undev.seed.total) ~ deviation * early.late + plot.treat * number.conspecifics + (1|site/plot.treat), family = binomial, data = delph.open) #modeling Delphinium proportion of developed seeds with lme4 package
#removed treat fixed effect (only open)
summary(Delph.glmer)
```
...
```{r checking delph overdispersion}
delph.prop.simulation<-simulateResiduals(fittedModel = Delph.glmer) #creating simulation for Delphinium model
...
Our Delphinium proportion data is overdispersed and zero-inflated.
...
There was an increased negative effect of conspecific density on individuals in the manipulated plot. 
...
delph.prop.treat.simulation<-simulateResiduals(fittedModel = Delph.treat.glmer) #creating simulation for Delphinium model
...
The pollen limitation model for Delphinium proportion data is overdispersed and zero-inflated.
