...
```{r fixing net NA issues, warning=FALSE, message=FALSE}
head(rmbl.net)
head(rmbl.net$date)
...
```{r flower counts, warning=FALSE,message=FALSE}
head(flower.counts)
...
```{r creating 0 in visit.mert, message=FALSE}
mert$bloom.wks<-abs(difftime(mert$start.estimate,mert$end.estimate, units = "weeks"))
as.numeric(mert$bloom.wks)
```
...
mert.visit.test<- mert2 %>% mutate(visitation = if_else(as.Date(date.y) %within% interval(as.Date(start.estimate),end=as.Date(end.estimate)), (intxns_per_flower/as.numeric(bloom.wks)), 0))
```
