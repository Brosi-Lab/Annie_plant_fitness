...
mert$absolute.position<-abs(mert$relative.position) #adding absolute value of relative position
```
...
delph$absolute.position<-abs(delph$relative.position) #adding absolute value of relative position
```
...
mert.glmmadmb<-glmmadmb(total.seeds ~ relative.position * plot.treat + (1|site), family = "nbinom2", data = mert) #Mertensia model for negative binomial data with glmmadmb package
...
Below is our first attempt at the mixed effects models without checking the distribution first.
...
Below is a previous attempt at modeling the data. We (incorrectly) used a model for Poisson-distributed data.
...
ggplot(delph, aes(x=delph$relative.position, y=delph$total.seeds, group=plot.treat)) +
  ...
  geom_smooth(method = "glm", formula= y~x) + ylim(-60,200)
```
...
```{r checking Mertensia proportion distribution}
mert.prop.pois<-fitdist(mert$prop.dev.seeds, "pois")
plot(mert.prop.pois)

mert.prop.nbinom<-fitdist(mert$prop.dev.seeds, "nbinom") #fitting Mertensia data to negative binomial distribution using fitdistplus
plot(mert.prop.nbinom) #plotting to see the fit
```

## Delphinium

```{r checking Delphinium proportion distribution}
plotdist(delph$prop.dev.seeds)
delph.prop.pois<-fitdist(delph$prop.dev.seeds, "pois", method = "mle", start = )
plot(delph.prop.pois)
```

# Mixed Effects Model using Absolute Value of Relative Position

## Mertensia

```{r checking absolute Mertensia dist}
